...
class PelajaranHuruf5Adapter (var listHurufFathahain: ArrayList<DataMenuPelajaran1Huruf>): RecyclerView.Adapter<PelajaranHuruf5Adapter.ViewHolder>() {
    ...
    override fun getItemCount(): Int {
        return listHurufFathahain.size
    }

    override fun onBindViewHolder(holder: ViewHolder, position: Int) {
        Glide.with(holder.itemView).load(listHurufFathahain[position].gambarPelajaran).into(holder.binding.imageView2)
        var grid = position / 5
        ...
        holder.binding.imageView2.setOnClickListener {
            val bundle = Bundle()
            bundle.putInt("id4", listHurufFathahain[position].id)
            bundle.putString("judul", "Pelajaran 5")
